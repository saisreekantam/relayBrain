...
- [x] **Explicit supersession signal** (`relay decide supersede|contradict`) — §6.5 always described two paths (explicit signal, narrow heuristic) but only the heuristic was ever built. Found via `scripts/benchmarkMemoryCorpus.js` (a hand-authored Decision/Goal/Evidence/Failure benchmark, architecturally inspired by exploring a real large repo but NOT extracted from it — see the script's header for exactly what that does and doesn't claim): a realistic query ("what approach was replaced?") had no way to succeed because nothing could express "B replaced A" for phrasing the antonym-pair heuristic doesn't catch. The CLI command reuses the materializer logic that already existed and was already tested — it just needed an entry point. Resolving the two decisions by exact text first, falling back to a substring match (`resolveDecisionRef`), turned out to matter immediately: a resolved decision's *stored* text always includes its date prefix as an internal detail, so requiring an exact match silently created an event pointing at a node that doesn't exist — caught by an actual CLI smoke test, not a unit test, fixed before it shipped.
- [x] **`node.type` removed from BM25 search text** — `nodeSearchText` included the literal type label ("Goal", "Decision"...), so any query containing a word like "goal" gave every node of that type a free, non-discriminating match, diluting the one that's actually relevant as a project accumulates more nodes of that type. A completely natural phrasing ("what's the GOAL of X") was the trigger. `node.kind` (Evidence's "benchmark"/"issue") was kept — it's a narrow, per-node value, not a type-wide label every sibling shares.
- [x] **Query-coverage discount** — plain BM25 has no notion of "what fraction of the query matched"; a single rare term with high IDF can score deceptively high even when the rest of the query is completely unmatched ("What blockchain integration does this project have?" scored 0.6 purely because one unrelated task happens to contain the word "integration"). Fixed the same way as the directory-match boost — an independent multiplier (fraction of distinct query terms present in the node's text), applied only to the lexical BM25 signal, never to embedding similarity (which is legitimately allowed to match with zero literal term overlap).
- [x] **History-line date-prefix stripped before truncation** — a superseded decision's stored text starts with its date ("2026-02-01 — ..."), which was eating most of `HISTORY_ENTRY_LEN`'s 24-character budget before any real content appeared in the compiled chain summary. Display-only fix (`stripDatePrefixForDisplay` in `relayContextCompiler.js`); the underlying `node.text` used for search/matching is untouched.

All five found via `scripts/benchmarkMemoryCorpus.js` (the type-search and coverage-discount fixes also re-verified against `scripts/benchmarkRelations.js` to confirm they didn't undo the earlier BM25 directory-match fix). Final result: 100% Recall@5 across 11 hand-labeled positive queries, MRR 0.624, 1/1 negative control correctly low-confidence.
